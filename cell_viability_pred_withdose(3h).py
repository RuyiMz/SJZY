#L1000-CTRP-withdose-3h dataset
import pandas as pd
import numpy as np
...
def getData():
    #1.Reading in data
    sig_viability_info = pd.read_table(r"LINCS_CTRP\LINCS_CTRP_dosetemp_3h.txt",sep="	",dtype=str)
    sig_viability_info_filter = sig_viability_info[['signature','cell_line','drug','dose','time','cell_viability']]
    sig_GE_info = pd.read_csv(r"LINCS_CTRP\LINCS_gene_expression_3h.csv",sep=",",dtype=str)
