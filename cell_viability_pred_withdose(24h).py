#L1000-CTRP-withdose-24h dataset
import pandas as pd
import numpy as np
...
def getData():
    #1.Reading in data
    sig_viability_info = pd.read_table(r"LINCS_CTRP\LINCS_CTRP_dosetemp_24h.txt",sep="	",dtype=str)
    sig_viability_info_filter = sig_viability_info[['signature','cell_line','drug','dose','time','cell_viability']]
    sig_GE_info = pd.read_csv(r"LINCS_CTRP\LINCS_gene_expression_24h.csv",sep=",",dtype=str)
    ...
    select_sig_viavility1 = pd.DataFrame(sig_viability).loc[:,'dose']
    select_sig_viavility2 = pd.DataFrame(sig_viability).loc[:,'5720':]
    select_sig_viavility2['dose'] = select_sig_viavility1
...
T = 75                  #The maximum number of iterations
...
#Saving the model
import pickle
gene_files = open(r"model_adjust\CTRP-L1000-24h_GeneSelection_1","w")
gene_files.write(str(selected_genes_dict))
gene_files.close()
pickle.dump(xgboost_best_model,open(r"model_adjust\CTRP-L1000-24h_XGBoost_1.dat","wb"))