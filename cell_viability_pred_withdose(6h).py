#L1000-CTRP-withdose-6h dataset
import pandas as pd
import numpy as np
...
def getData():
    #1.Reading in data
    sig_viability_info = pd.read_table(r"LINCS_CTRP\LINCS_CTRP_dosetemp_6h.txt",sep="	",dtype=str)
    sig_viability_info_filter = sig_viability_info[['signature','cell_line','drug','dose','time','cell_viability']]
    sig_GE_info = pd.read_csv(r"LINCS_CTRP\LINCS_gene_expression_6h.csv",sep=",",dtype=str)
...
#Saving the model
import pickle
gene_files = open(r"model_adjust\CTRP-L1000-6h_GeneSelection_1","w")
gene_files.write(str(selected_genes_dict))
gene_files.close()
pickle.dump(xgboost_best_model,open(r"model_adjust\CTRP-L1000-6h_XGBoost_1.dat","wb"))