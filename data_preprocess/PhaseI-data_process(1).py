# -*- coding: utf-8 -*-
"""
Created on Sun Aug 25 13:41:57 2019

@author: 52665
"""

#Processing the LINCS-L1000 Phase I data
...
LINCS_PhaseII_CTRP_file = open("LINCS_PhaseI_CTRP/LINCS_PhaseI_CTRP_24h.txt","w")
LINCS_PhaseII_CTRP_logfile = open("LINCS_PhaseI_CTRP/LINCS_PhaseI_CTRP_log_24h.txt","w")
...
sig_info = pd.read_csv("LINCS\LINCS Phase I Data--GEO GSE92742\GSE92742_Broad_LINCS_sig_info.txt", sep="\t")
#1.1 Data processing
for sig_info_index in range(len(sig_info)):
    dose_value = str(sig_info.at[sig_info_index,"pert_dose"]).strip(" um")
    sig_info.at[sig_info_index,"pert_dose"] = dose_value
#print(sig_info)
sig_info_exclude_dose = sig_info[sig_info["pert_dose"] != "-666"]
sig_info_exclude_dose = sig_info_exclude_dose[sig_info["pert_dose"] != "0.0"]
...
for CTRP_index in range(len(viability_cpd_ccl_info)):
    ...
    for LINCS_index in range(len(sig_info_exclude_dose)):
        ...
        if ((CTRP_cell_line == LINCS_cell_line) and (CTRP_drug == LINCS_drug)):
            ...
            LINCS_PhaseII_CTRP_file.write(str(sig_info_exclude_dose.at[LINCS_index,'pert_dose'])+"\t")
